#!/usr/bin/env python3
"""
Cat Face Detection Training Script (320x320)
使用320x320输入尺寸进行训练，速度更快、显存占用更少

使用方法（简化版）:
    python train_cat_face_320.py                    # 使用默认配置
    python train_cat_face_320.py --epochs 10        # 覆盖参数
    python train_cat_face_320.py --quant             # 启用量化训练
...
def get_config_value(args, config, key, default=None):
    ...
    if cmd_value is not None and (not isinstance(cmd_value, bool) or cmd_value):  # bool 类型特殊处理
        return cmd_value
...
def add_quantization(model, config_file, stage='quant'):
    ...
    if stage == 'constrain':
        ...
        return init(
            ...
            disable_submodel=('*9.m',)  # 跳过 SPPF 模块中的 MaxPool
        )
...
def main(args):
    ...
    name = get_config_value(args, config, 'name', 'cat_face_yolo26n_train_320')
    ...
    # 数据集和模型配置
    dataset_file = get_config_value(args, config, 'dataset', 'cat_face_320.yaml')
    model_file = get_config_value(args, config, 'model', 'yolo26n_cat.yaml')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Cat Face Detection Training',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python train_cat_face_320.py                    # 使用默认配置
  python train_cat_face_320.py --epochs 10         # 覆盖训练轮数
  python train_cat_face_320.py --device 0          # 使用GPU 0
  python train_cat_face_320.py --quant             # 启用量化训练
  python train_cat_face_320.py --quant --quant-stage constrain  # 仅约束训练
        """
    )
    # 配置文件（可选）
